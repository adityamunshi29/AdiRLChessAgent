...
model = load_model(path.join('models','adi12_2'))
player = agent.Agent(lr=0.001, network='big', model=model)
player.fix_model()
learner = learn.TD_search(env, player, gamma=0.8, search_time=1.5)
node = tree.Node(learner.env.board, gamma=learner.gamma)
#TD Learning: predicts reward at next timestep, when received, compares to prediction and computes error-- drives learning.
