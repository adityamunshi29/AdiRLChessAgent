...
class TD_search(object):
    ...
    def __init__(self, env, agent, gamma=0.9, search_time=1, memsize=2000, batch_size=256, temperature=1):
        ...
        self.temperature = temperature #slow decrease in the probability of accepting worse solutions
    ...
    def learn(self, iters=40, c=5, timelimit_seconds=3600, maxiter=80, model_name='adimodel12'):
        """
        ...
        if k%10 ==0:
            ...
            self.agent.model.save(model_name + str(k))
    ...
    def play_game(self, k, maxiter=80):
        ...
        while not episode_end:
            ...
            if self.env.board.turn:
                ...
                # Monte Carlo TS after kth game iteration
                start_mcts_after = -1
                if k > start_mcts_after:
                    tree = self.mcts(tree)
                    # Step the best move
                    max_move = None
                    max_value = np.NINF #negativeinfinity
                ...
                max_move = None
                min_value = np.PINF #alternatively use really small number-- ensure that this is as small as possible in order to increment maxvalue with r/ to time
                for move in self.env.board.generate_legal_moves():
                    self.env.step(move)
                    if self.env.board.result() == "0-1":
                        max_move = move
                        self.env.board.pop() #i.e in a stack, popping the last element
                        self.env.init_layer_board() #at the end of the game resets board.
                        break
                    successor_state_value_opponent = self.env.opposing_agent.predict( #not material score. this is score for the move
                        np.expand_dims(self.env.layer_board, axis=0))
                    if successor_state_value_opponent < min_value:
                        max_move = move
                        min_value = successor_state_value_opponent

                    self.env.board.pop()
                    self.env.init_layer_board()

            if not (self.env.board.turn and max_move not in tree.children.keys()) or not k > start_mcts_after:
                tree.children[max_move] = Node(gamma=0.9, parent=tree) #applying discount factor
