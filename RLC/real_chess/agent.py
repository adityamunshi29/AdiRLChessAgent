...
class Agent(object):
    ...
    def __init__(self, lr=0.003, network='big', model=None):
    ...
    def TD_update(self, states, rewards, sucstates, episode_active, gamma=0.9):
        ...
        # If gpu runs out of memory set minibatch to 16
        self.model.fit(x=states, y=V_target, epochs=3, verbose=1, batch_size=32)
    ...
    def adi_network(self):

        layer_state = Input(shape=(8,8,8))
        f_ls = Flatten()(layer_state)
        dense0 = Dense(256, activation='sigmoid')(f_ls)
        dense1 = Dense(64, activation='sigmoid')(dense0)
        dense2 = Dense(10, activation='sigmoid')(dense1)
        value_head = Dense(1)(dense2)

        self.model = Model(inputs=layer_state,
                           outputs=value_head)
        self.model.compile(optimizer='ad'
                                     'am',
                           loss=mean_squared_error
                           )
        return True